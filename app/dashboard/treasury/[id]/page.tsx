  ...
  return (
    <div className="px-4 py-8 sm:p-8 max-w-[1440px] mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl sm:text-[30px] font-bold tracking-[-0.02em] text-[#1e3a8a] truncate">
          {account?.name ?? 'Cuenta'}
        </h1>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-[#eef2f7] p-6 mb-6">
          <div className="flex flex-wrap gap-8 items-end">
            <div>
              ...
              <p className={`text-3xl sm:text-[40px] font-bold tracking-tight leading-none ${balance.estimated_balance >= 0 ? 'text-[#1e3a8a]' : 'text-rose-600'}`}>
                €{fmt(balance.estimated_balance)}
              </p>
            </div>
            <div className="flex flex-wrap gap-x-6 gap-y-3 pb-1 border-t border-slate-50 pt-4 sm:border-0 sm:pt-0">
              <div>
                <p className="text-[10px] text-slate-400 mb-0.5">Saldo inicial</p>
                <p className="text-[14px] sm:text-[15px] font-semibold text-slate-600">€{fmt(balance.opening_balance)}</p>
              </div>
              <div className="hidden sm:block text-slate-300 self-center text-lg">+</div>
              <div>
                <p className="text-[10px] text-slate-400 mb-0.5">Entradas totales</p>
                <p className="text-[14px] sm:text-[15px] font-semibold text-emerald-700">€{fmt(balance.total_income)}</p>
              </div>
              <div className="hidden sm:block text-slate-300 self-center text-lg">−</div>
              <div>
                <p className="text-[10px] text-slate-400 mb-0.5">Salidas totales</p>
                <p className="text-[14px] sm:text-[15px] font-semibold text-rose-600">€{fmt(balance.total_expenses)}</p>
              </div>
            </div>
          </div>
        </div>
