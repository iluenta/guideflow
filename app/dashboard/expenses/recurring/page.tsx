    ...
    <div className="px-4 py-8 sm:p-8 max-w-[1440px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6 md:mb-8">
        ...
        {canCreate && (
          <button
            onClick={openCreate}
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[#1e3a8a] text-white text-sm font-semibold hover:bg-[#1e3a8a]/90 transition-colors shadow-sm"
          >
      ...
      {loading ? (
        ...
        <>
          {/* Desktop Table */}
          <div className="hidden md:block bg-white rounded-2xl border border-[#eef2f7] overflow-hidden">
            {/* Header tabla */}
          ...
          <div className="md:hidden space-y-4">
            {templates.map(tpl => {
              const nextGen = getNextGenDate(tpl)
              return (
                <div key={tpl.id} className="bg-white rounded-2xl border border-[#eef2f7] p-5 space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="min-w-0 flex-1">
                      <p className="text-[15px] font-bold text-slate-800 truncate">{tpl.name}</p>
                      <p className="text-[12px] text-slate-400 truncate">{tpl.property_name}</p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0 ml-4">
                      {canEdit && (
                        <button
                          onClick={() => openEdit(tpl)}
                          className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-[#eef2fb] hover:text-[#1e3a8a]"
                        >
                          <Pencil className="h-3.5 w-3.5" />
                        </button>
                      )}
                      {canDelete && (
                        <button
                          onClick={() => handleDelete(tpl)}
                          className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-rose-50 hover:text-rose-500"
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </button>
                      )}
                  ...
                  <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                    <div>
                      <p className="text-[10px] font-mono uppercase text-slate-400 mb-0.5">Categoría</p>
                      <p className="text-[12px] text-slate-600 font-medium">{EXPENSE_CATEGORY_LABELS[tpl.category]}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-mono uppercase text-slate-400 mb-0.5">Frecuencia</p>
                      <p className="text-[12px] text-slate-600 font-medium">{FREQ_LABELS[tpl.frequency]} (Día {tpl.day_of_period})</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-mono uppercase text-slate-400 mb-0.5">Importe</p>
                      <p className="text-[14px] font-bold text-slate-800">€{fmt(tpl.estimated_amount)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-mono uppercase text-slate-400 mb-0.5">Próxima Gen.</p>
                      <p className="text-[12px] text-slate-500 font-medium">{nextGen ?? '—'}</p>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                    <span className="text-[12px] font-medium text-slate-500">Activo</span>
                    <Switch
                      checked={tpl.is_active}
                      onCheckedChange={() => handleToggle(tpl)}
                      disabled={toggling === tpl.id || !canEdit}
...
function getNextGenDate(tpl: RecurringExpenseTemplate): string | null {
  ...
  } else if (tpl.frequency === 'biannual') {
    next = new Date(base.getFullYear(), base.getMonth() + 6, tpl.day_of_period)
  } else {
