        ...
        <div className="flex flex-col sm:grid sm:grid-cols-2 gap-4">
          {methods.map(m => {
            ...
            return (
              <div
                key={m.id}
                className={`bg-white border border-[#eef2f7] rounded-[18px] p-4 sm:p-6 shadow-sm transition-all ${m.is_active ? 'hover:border-[#3b5bbd] hover:shadow-[0_12px_28px_-14px_rgba(30,58,138,0.18)]' : 'opacity-60 bg-[#f1f4f8]'}`}
              >
                <div className="flex justify-between items-start mb-5 gap-2">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-[15px] sm:text-[16px] font-semibold text-slate-800 tracking-[-0.01em] truncate">{m.name}</h3>
                      <p className="text-[11px] text-slate-400 font-mono truncate">{m.code}</p>
                    </div>
                  </div>
                  {canEdit && (
