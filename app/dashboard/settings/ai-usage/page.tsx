...
import { RefreshCw, Zap, Calendar as CalendarIcon, ChevronRight } from "lucide-react";
...
export default function AiUsagePage() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error("Error al cargar las analíticas de IA");
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="max-w-[1440px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-extrabold tracking-tight text-[#000d37] font-manrope">
              Uso IA · <span className="text-blue-600">{activePropName}</span>
            </h1>
          </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <IaCostChart dailyData={dailyIaData} opData={opIaData} isLoading={isLoading} />
          </div>
          <div>
            <GuestInsightsBar data={sectionUsage} isLoading={isLoading} />
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <IaDetailLog entries={iaLogEntries} isLoading={isLoading} />
        </section>
