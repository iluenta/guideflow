...
export const DashboardTopbar = ({ onMenuClick, profile, onSignOut }: TopbarProps) => {
  ...
  useEffect(() => {
    function handler(e: KeyboardEvent) {
      // Ctrl+/ o ⌘/ — no conflicta con atajos del navegador
  ...
  return (
    <>
      <header className="sticky top-0 z-40 h-[72px] bg-landing-bg/85 backdrop-blur-xl border-b border-landing-rule-soft flex items-center justify-between px-5 sm:px-8">
        ...
        {/* Search bar — desktop only, opens modal on click */}
        <div className="hidden md:flex flex-1 max-w-[420px] mx-10">
          <button
            type="button"
            onClick={() => setSearchOpen(true)}
            className="w-full flex items-center gap-3 bg-landing-bg-deep border border-transparent rounded-full py-2.5 px-4 text-sm text-landing-ink-mute hover:border-landing-navy-soft hover:bg-white transition-all"
          >
            <Search className="h-4 w-4 shrink-0" />
            <span className="flex-1 text-left">Buscar propiedades, reservas...</span>
            <kbd className="font-jetbrains text-[10px] px-2 py-1 bg-white border border-landing-rule rounded-md shrink-0">Ctrl+/</kbd>
          </button>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-3">
          ...
          <button
            onClick={() => setSearchOpen(true)}
            className="md:hidden w-10 h-10 rounded-full flex items-center justify-center text-landing-ink-soft hover:bg-landing-bg-deep transition-all"
            aria-label="Buscar"
          >
