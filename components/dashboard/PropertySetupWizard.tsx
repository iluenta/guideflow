...
export function PropertySetupWizard({ propertyId, tenantId, onSuccess }: PropertySetupWizardProps) {
    ...
    // Extract property ID from URL pathname if not provided in props
    // This handles case when we're on /dashboard/properties/[id]/setup
    const urlPropertyId = pathname?.match(/\/dashboard\/properties\/([^\/]+)/)?.[1]
    const effectivePropertyId = propertyId || urlPropertyId || null
    ...
    // Flag: true cuando la carga inicial de datos de contexto ha terminado
    ...
    useEffect(() => {
        ...
        // Evitar recalculo si la dirección es exactamente la misma que la ?ltima geocodificada
    ...
    useEffect(() => {
        ...
        // Esperar a que el contexto cargado esté disponible antes de verificar datos existentes
        // Usamos un timer más largo (3.5s) para garantizar que propertyContext haya terminado de cargar
        const timer = setTimeout(() => {
            // Read access data directly to decide if we need AI fill
            const accessData = data.access
            const hasTransportData = !!(
                accessData.from_airport ||
                accessData.from_train ||
                accessData.parking?.info ||
                (Array.isArray(accessData.nearby_transport) && accessData.nearby_transport.length > 0)
            )

            if (hasTransportData) {
                // Si ya hay datos previos, mostrar alerta para regenerar
                // Solo si el usuario está en el tab de acceso
                setShowRegenerateAlert(true)
            } else if (!aiLoading) {
                // No hay datos y no está cargando → generar automáticamente
    ...
    useEffect(() => {
        ...
        if (tab && steps.includes(tab) && tab !== activeTab) {
            // Si no hay propertyId, forzar vuelta al paso property
    ...
    const handleTabChange = (value: string) => {
        // GATE: no permitir avanzar si la propiedad no está guardada aún
        if (!effectivePropertyId && value !== 'property') {
            toast({
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            if (stepData && activeTab !== 'visual-scanner') {
                // FIX: el tab 'appearance' debe guardarse como 'branding' en property_branding,
                // no como 'appearance' en property_context.
    ...
    useEffect(() => {
        ...
        async function loadData() {
            ...
            if (propDetails) {
                ...
                // Do not set lastAIPositionRef here so the transport useEffect can call
                // handleAIFill('transport') when the user opens the Access tab with loaded coordinates.
    ...
    const handleAIFill = async (section: string, category: string = 'todos') => {
        // GATE: Avoid re-entrancy if already loading this section
        if (aiLoading === section) return;

        const isTransport = ['transport', 'plane', 'train', 'road'].includes(section);

        // Immediate position lock to prevent race conditions from useEffect/setTimeout
        if (isTransport && geocodingResult) {
            const currentPosKey = `${geocodingResult.lat},${geocodingResult.lng}`;
            if (currentPosKey === lastAIPositionRef.current && section === 'transport') {
                console.log('[AI-FILL] Skipping: Position already processed and no section override');
                // return; // Don't return yet, let the state check decide, but this is a good secondary guard
            }
        ...
        if (!currentPropId) {
            toast({
                title: '⚠️ Guarda el Paso 1 primero',
                description: 'Necesitamos que la propiedad esté guardada antes de generar contenido con IA.',
                variant: 'destructive'
            })
            return
        }
        ...
        const finalAddressToUse = data.access.full_address || property?.full_address || '';

        // Para transport sin propertyId: enviar address-only (el API lo soporta)
        const payload: any = {
            propertyId: currentPropId || 'address-only',
            section,
        ...
        try {
            ...
            if (!res.ok) {
                toast({ title: 'Error IA', description: result?.error || 'Error en el servidor', variant: 'destructive' })
                return
            }
            ...
            if (isTransport) {
                setData((prev: any) => {
                    ...
                    // Smart merge: only update fields that are NOT null or empty
                    Object.keys(incoming).forEach((key) => {
                        const val = incoming[key];
                        const isValEmpty = val === null || val === undefined ||
                    ...
                    return {
                        ...
                        access: {
                            ...mergedAccess,
                            full_address: prev.access.full_address || '' // Keep address
                        }
                    };
                });
            } else if (section === 'dining') {
                const rawRecs = Array.isArray(result.recommendations)
                    ? result.recommendations
                    : Array.isArray(result) ? result : [];
                const incomingRecs = rawRecs.map((r: any, i: number) => ({
                    ...r,
                ...
                const uniqueIncoming = incomingRecs.reduce((acc: any[], current: any) => {
                    const exists = acc.find(item => item.name && String(item.name).toLowerCase().trim() === String(current.name).toLowerCase().trim());
                    if (!exists) return [...acc, current];
                    return acc;
                }, []);

                // Deduplicate against existing: we need current dining state (setData callback below uses prev)
                setData((prev: any) => {
                    const existingDining = prev.dining || [];
                    const newRecs = uniqueIncoming.filter((nr: any) =>
                        !existingDining.some((er: any) => er.name && String(er.name).toLowerCase().trim() === String(nr.name).toLowerCase().trim())
                    );
                    if (newRecs.length === 0 && rawRecs.length > 0) {
                        setTimeout(() => toast({ title: 'Recomendaciones existentes', description: 'Las sugerencias generadas ya están en tu lista.' }), 0);
                        return prev;
                    }
                    if (newRecs.length === 0 && rawRecs.length === 0) {
                        setTimeout(() => toast({ title: 'Sin recomendaciones', description: 'La IA no devolvió datos. Prueba de nuevo o cambia la categoría.', variant: 'destructive' }), 0);
                        return prev;
                    }
                    return { ...prev, dining: [...existingDining, ...newRecs] };
                });
                ...
                const raw = result.emergency_contacts || [];
                const normalized = raw.map((c: any) => ({
                    id: c.id || crypto.randomUUID(),
                    name: c.name ?? c.service ?? c.nombre ?? c.title ?? '',
                    phone: c.phone ?? c.number ?? c.telefono ?? c.tel ?? '',
                    address: c.address ?? c.direccion ?? c.direccion_exacta ?? '',
                    distance: c.distance ?? c.distancia ?? c.tiempo ?? '',
                    type: ['salud', 'farmacia', 'policia', 'bomberos'].includes(c.type) ? c.type : (c.tipo && ['salud', 'farmacia', 'policia', 'bomberos'].includes(c.tipo) ? c.tipo : 'salud')
                })).filter((c: any) => c.phone || c.name);
                setData((prev: any) => ({
                    ...prev,
                    contacts: {
                        ...prev.contacts,
                        emergency_contacts: normalized
                    }
                }))
            }
                                ...
                                <TabsContent value="visual-scanner" className="mt-4 w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    <Card className="border-none shadow-lg bg-white rounded-2xl overflow-hidden">
                                        <CardContent className="p-6">
                                            <VisualScanner
                                                key={propertyId || property?.id || 'no-property'}
