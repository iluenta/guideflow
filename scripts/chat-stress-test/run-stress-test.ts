...
const envPath = path.resolve(__dirname, "../../.env.stress");
if (fs.existsSync(envPath)) {
  const envConfig = config({ path: envPath }).parsed || {};
  // Si dotenv falló, intentamos manual
  if (!process.env.GEMINI_API_KEY && envConfig.GEMINI_API_KEY) {
...
if (CONFIG.geminiApiKey) {
  console.log(`   [CONFIG] Gemini API Key cargada ✓ (${CONFIG.geminiApiKey.slice(0, 5)}...)`);
} else {
  console.warn(`   [CONFIG] Gemini API Key NO cargada. Evaluación desactivada.`);
}
...
Temas esperados: ${scenario.expected_topics?.join(", ") || "ninguno"}
...
Devuelve SOLO este JSON:
{
  ...
  try {
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${CONFIG.geminiApiKey}`,
      {
        ...
        body: JSON.stringify({
          ...
          systemInstruction: {
            parts: [{ text: "Eres un evaluador de calidad de IA. Responde SOLO con JSON." }]
          }
        }),
      }
    );
    ...
    if (!res.ok) {
      console.error(`   [DEBUG] Gemini Eval Error: ${res.status}`, JSON.stringify(data.error));
      return { eval_notes: `Error Gemini ${res.status}` };
    }
    ...
    return { eval_notes: `Error: ${err.message}` };
  }
}
...
async function main() {
  ...
  if (CONFIG.filterProperty) {
    // Try to load token from properties.json if it was previously fetched
    let accessToken: string | undefined;
    let name = `Property ${CONFIG.filterProperty.slice(0, 8)}`;
    if (fs.existsSync(propertiesPath)) {
      const allProps: Property[] = JSON.parse(fs.readFileSync(propertiesPath, "utf-8"));
      const match = allProps.find((p) => p.id === CONFIG.filterProperty);
      if (match) {
        accessToken = match.accessToken;
        name = match.name ?? name;
      }
    }
    properties = [{ id: CONFIG.filterProperty, name, accessToken }];
