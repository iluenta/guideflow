-- Fix: apply threshold AFTER priority_boost so high-priority chunks (FAQs) are not
-- discarded before boosting. A FAQ with similarity 0.27 × 1.2 boost = 0.324, which
-- is useful, but the old WHERE filtered it out before boost was applied.
...
BEGIN
  ...
  WHERE ce.property_id = p_property_id
    AND (1 - (ce.embedding <=> query_embedding)) * (CASE
      WHEN ce.source_type = 'faq' THEN 1.2
      WHEN ce.source_type = 'recommendation' THEN 1.1
      WHEN ce.source_type = 'context' THEN 1.0
      WHEN ce.source_type = 'manual' THEN 0.9
